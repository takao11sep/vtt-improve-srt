...
# -*- coding: utf-8 -*-
"""
VTTファイルをGemini APIで高精度校正してSRTに変換するスクリプト
使い方: python3 process_vtt_gemini.py <vttファイルのパス>
...
def call_gemini_api(entries_batch, api_key):
...
def process_batch(entries, batch_num, output_dir, api_key):
    ...
    print(f"  → Gemini API 呼び出し中...")

    # Gemini APIで校正
    corrected = call_gemini_api(entries, api_key)
...
def main():
    """メイン処理"""
    print("=" * 60)
    print("VTT to SRT Converter - Gemini AI 高精度校正版")
    print("=" * 60)
    ...
    print(f"\nGemini API バッチ処理を開始します（{batch_size}エントリ/バッチ）")
